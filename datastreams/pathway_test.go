...
func TestPathway(t *testing.T) {
	t.Run("test SetCheckpoint", func(t *testing.T) {
		...
		p := newPathway(start, NewCheckpointParams(0))
		p = p.setCheckpoint(middle, NewCheckpointParams(0), []string{"edge-1"})
		p = p.setCheckpoint(end, NewCheckpointParams(0), []string{"edge-2"})
